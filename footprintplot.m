...
function [H]=footprintplot(Nf,X,leng,angle_deg,orientation,tdomain,zscale,name,showpictures)
if showpictures
    plotphase=0;
...
if orientation=='x'
    yleng=leng;
    xleng=leng/deg2rad(angle_deg);
elseif orientation=='y'
    yleng=leng/deg2rad(angle_deg);
    xleng=leng;
else
    error('"angle_deg" should bee "x" or "y"');
end
    [Mx,My,N]=size(X);
    dx=xleng/Mx;
    dy=yleng/My;
...
if N==1
    ...
    if tdomain
        ...
        error('not tested for frequency domain');
        i1=round(N/2);
        i2=i1+1;
...
%                 h1=subplot(2,2+adc,1);
                ...
                imagesc(xscale,yscale,abs(X).^2./dS./10000);
                drawnow
                axis tight;
                if P0~=0
...
%                     if ~plotphase
%                         h4=subplot(2,2+adc,4+adc);
%                         lzprojection=reshape(sum(sum(abs(Xo).^2,1),2),1,[]); %lambda/z projection
...
%                     imagesc(zscale,xscale,reshape(sum(abs(Xo).^2,2),Mx,N));
%                     if tdomain==1
%                         xlabel('z [m]');
%                     else
%                         xlabel('wavelength [m]');
...
%                     imagesc(zscale,yscale,reshape(sum(abs(Xo).^2,1),My,N)); % <- problem
% 
%                     if tdomain==1
%                         xlabel('z [m]');
%                     else
