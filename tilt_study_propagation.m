...
%     X1=prop_TF(X, leng_i, xlamds,slitpos);
%     [H{6}]=fieldplot3d(6,X1,leng_i,0,Xlamdsf,'slit frequency domain',1);
%     [H{601}]=fieldplot3d(601,ifft(ifftshift(X1,3),[],3),leng_i,0,Zf,'slit time domain',1);
%     clear X1
...
% %         X=mirror(X,leng_i,K,z3_t_offset-slitpos,z4_t,R_m_tang,R_m_tang,Teta_m_i,'y',aberrationsincluded);
    ...
    for i=1:Nf
    X(:,:,i)=mirror(X(:,:,i),leng_i,xlamds,z3_t_offset-slitpos,z4_t,R_m_tang,R_m_tang,Teta_m_i,'y',aberrationsincluded);
    X(:,:,i)=aperture(X(:,:,i),leng_i,m2_length*Teta_m_i,'y');
    end
    ...
    X=prop_TF(X, leng_u2, xlamds,10); %remove!!!
...
%     end
    ...
    break
%% Export
    fieldexport(cat(3,Xt,single(zeros(151,151,npad)).*(1+1i)),'1000_u1_filtered_u8.dfl');
    clear all