...
z1=(1787+2.37*energy)/1000; %sourse distance
%z1=(1787+2.37*energy+3610)/1000; %sourse distance with U8 removed

Teta_g_d=acos(cos(Teta_g_i)-xlamds/gr_sigma);%4.39 for 500 eV, 3.18 for 1keV !!!
...
f_g_s=R_g_sag/(sin(Teta_g_i)+sin(Teta_g_d));
disp(['f_g_s=',num2str(f_g_s),'m']);
%% tangential focal distance calculation

% vls contribution
f_g_t_vls=gr_sigma^2*sin(Teta_g_d)^2/xlamds/gr_alpha; %!!! original!!!!
%f_g_t_vls=gr_sigma^2/xlamds/gr_alpha;

% curvature contribution
f_g_t_c=R_g_tang/(1/Teta_g_d+Teta_g_i/Teta_g_d^2); %tangential focal distance due to curvature

% final tangential focal distance
f_g_t=1/(1/f_g_t_vls+1/f_g_t_c);
%f_g_t=1.07;

zr_prime=zr*(Teta_g_d/Teta_g_i)^2;
z1_prime=z1*(Teta_g_d/Teta_g_i)^2;
w1_prime=w1*(Teta_g_d/Teta_g_i);

disp(['f_g_t=',num2str(f_g_t),'m,  f_g_t_c=',num2str(f_g_t_c),'m,  f_g_t_vls=',num2str(f_g_t_vls),'m']);
%disp(['z1''=',num2str(z1_prime),'m,  s1''=',num2str(w1_prime/sqrt(2)*1e6),'mikr,   zr''=',num2str(zr_prime),'m']);
%% propagation 1
...
% w2_t_simplified=w1*Teta_g_i/Teta_g_d*f_g_t/sqrt(z1^2+zr^2);
...
N_gr=w1*sqrt(1+(z1/zr)^2)/Teta_g_i/gr_sigma;
w_gr=w1*sqrt(1+(z1/zr)^2);
disp(['w_gr=',num2str(w_gr*1e6)]); %to remove
%%
...
distr_g=exp(-2.*(dlpl.*f_g_t.*xlamds./Teta_g_d./gr_sigma./w_slit).^2); %gauss distribution
distr_ft=double(abs(dlpl)<ds*Teta_g_d*gr_sigma/f_g_t/xlamds/2);         %mono distribution
...
res_fwhm_ft=ds*Teta_g_d*gr_sigma/f_g_t/xlamds;
res_fwhm_eff=findFWHM(linspace(-sdx*2,sdx*2,snx*2-1),distr_eff);

%%% tmp
%res_fwhm_eff=sqrt(2*log(2))*w_slit*Teta_g_d*gr_sigma/f_g_t/xlamds;
%%%
...
zzres_arg=salpha';
zzresfunc=(res.^-1)';
%resolution_slit=1/(f_g_t.*xlamds./Teta_g_d./gr_sigma./w_slit)
%resolution_slit=Teta_g_i*gr_sigma/pi/(w1*sqrt(1+(z1/zr)^2));
%!!! tmp
%1/resolution_slit
%resolution_slit=w_slit*Teta_g_d*gr_sigma./xlamds/f_g_t;
...
C_30=(Teta_m_i^2/z3_t-Teta_m_i/R_m_tang)/2/z3_t+(Teta_m_i^2/z4_t-Teta_m_i/R_m_tang)/2/z4_t;
C_30*2*pi/xlamds*(8e-5)^3/Teta_m_i^3;