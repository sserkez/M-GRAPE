...
opt_parms_5;
%opt_parms_LCLS; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% REMOVE!

u2_plot_size_lim=[3 15];
u2_plot_long_lim=[-2 1];
u2_plot_size_prop_lim=[0 300];
...
ang_disp_U2_out=linspace(1,1,size(ienergy,2));
...
gafoc; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% REMOVE!
    ...
    w1_out(index)=w1;
    N_gr_out(index)=N_gr;
    ang_disp_U2_out(index)=ang_disp_U2;
...
if showpictures
    ...
    subplot(2,1,2);
    hold all
    plot(ienergy,1e6.*w1_out./2);
    hold off
    xlabel('Energy [eV]');
    ylabel('Waist size _{\sigma} [\mum]');
    ...
    subplot(3,1,2);
    hold all
    plot(ienergy,1e6.*w2_t_out./2);
    hold off
    xlabel('energy, eV');
    ylabel('Waist size \sigma [um]');
    subplot(3,1,3);
    hold all
    plot(ienergy,1e6.*w2_t_out./2.*sqrt((z2_t_out-slitpos).^2./zr_slit_out.^2+1));
    hold off
    xlabel('energy, eV');
    ylabel('Spot at slit size \sigma [um]');
...
%     plot(ienergy,z4_t_geom_out,'marker','*','linestyle','--','color','m','MarkerSize',7);
    ...
    subplot(3,1,2);
    hold all
    plot(ienergy,1e6.*w4_s_out./2,'linestyle','-','color','r');
    plot(ienergy,1e6.*w4_t_out./2,'linestyle','--','color','r');
    plot(ienergy,1e6.*w1_out./2,'marker','*','color','k');
    hold off
    xlabel('energy, eV');
    ylabel('Waist size \sigma [um] s(-), t(--), orig(*)');

    subplot(3,1,3);
    hold all
    plot(ienergy,1e6.*w4_s_out./2.*sqrt(1+((z4_s_out)./(pi.*w4_s_out.^2./xlamdsout)).^2),'linestyle','-','color','r');
    plot(ienergy,1e6.*w4_t_out./2.*sqrt(1+((z4_t_out)./(pi.*w4_t_out.^2./xlamdsout)).^2),'linestyle','--','color','r');
    plot(ienergy,1e6.*w4_s_out./2.*sqrt(1+((z4_s_out-1)./(pi.*w4_s_out.^2./xlamdsout)).^2),'linestyle','-','color','b');
    plot(ienergy,1e6.*w4_t_out./2.*sqrt(1+((z4_t_out-1)./(pi.*w4_t_out.^2./xlamdsout)).^2),'linestyle','--','color','b');
    plot(ienergy,1e6.*w4_s_out./2.*sqrt(1+((z4_s_out-2)./(pi.*w4_s_out.^2./xlamdsout)).^2),'linestyle','-','color','g');
    plot(ienergy,1e6.*w4_t_out./2.*sqrt(1+((z4_t_out-2)./(pi.*w4_t_out.^2./xlamdsout)).^2),'linestyle','--','color','g');
    plot(ienergy,1e6.*w1_out./2,'marker','*','color','k');
    hold off
    xlabel('energy, eV');
    ylabel('Spot \sigma [um] at 0m-r 1m-b 2m-g');
...
if showpublicationpictures
    ...
    subplot(2,1,2);
    hold all
    plot(ienergy,1e6.*w1_out./2,'linestyle','-','linewidth',2,'color','r');
    hold off
    if numel(ienergy)>1
        xlim([min(ienergy) max(ienergy)]);
    end
    xlabel('Photon energy [eV]');
    ylabel('\sigma_{int} waist size [\mum]');
    ...
    subplot(2,1,2);
    hold all
    plot(ienergy,1e6.*w2_t_out./2,'linestyle','-','linewidth',2,'color','r');
    plot(ienergy,1e6.*w2_t_out./2.*sqrt((z2_t_out-slitpos).^2./zr_slit_out.^2+1),'linestyle','--','linewidth',1.5);
    hold off
    if numel(ienergy)>1
        xlim([min(ienergy) max(ienergy)]);
    end
    xlabel('Photon energy [eV]');
    ylabel('\sigma_{int} size [\mum]');
...
%     ylabel('Illuminated grooves');
    ...
    ylim([-1 10]);
    ylim(u2_plot_long_lim);
    ylabel('Waist position [m]');
    xlabel('Photon energy [eV]');
    
    subplot(2,1,2);
    hold all
    plot(ienergy,1e6.*w4_s_out./2,'linestyle','-','color','r','linewidth',2);
    plot(ienergy,1e6.*w1_out./2,'marker','*','color','k');
    hold off
    if numel(ienergy)>1
        xlim([min(ienergy) max(ienergy)]);
    ...
    subplot(2,1,2);
    hold all
    plot(ienergy,1e6.*w4_t_out./2,'linestyle','-','color','r','linewidth',2);
    plot(ienergy,1e6.*w1_out./2,'marker','*','color','k');
    hold off
    if numel(ienergy)>1
        xlim([min(ienergy) max(ienergy)]);
...
clf
    subplot(1,2,1);
    hold all
    plot(ienergy,1e6.*w4_s_out./2.*sqrt(1+((z4_s_out)./(pi.*w4_s_out.^2./xlamdsout)).^2),'linestyle','-','color','r','linewidth',1.5);
    plot(ienergy,1e6.*w4_s_out./2.*sqrt(1+((z4_s_out-2)./(pi.*w4_s_out.^2./xlamdsout)).^2),'linestyle','--','color','b','linewidth',1.5);
    plot(ienergy,1e6.*w4_s_out./2.*sqrt(1+((z4_s_out-5)./(pi.*w4_s_out.^2./xlamdsout)).^2),'linestyle','-.','color','g','linewidth',1.5);
    plot(ienergy,1e6.*w1_out./2,'marker','*','color','k');
    hold off
    ylim(u2_plot_size_prop_lim);
    if numel(ienergy)>1
        xlim([min(ienergy) max(ienergy)]);
    end
    xlabel('Photon energy [eV]');
    ylabel('\sigma_{int} spot size [\mum]');
    
    
    subplot(1,2,2);
    hold all
    plot(ienergy,1e6.*w4_t_out./2.*sqrt(1+((z4_t_out)./(pi.*w4_t_out.^2./xlamdsout)).^2),'linestyle','-','color','r','linewidth',1.5);
    plot(ienergy,1e6.*w4_t_out./2.*sqrt(1+((z4_t_out-1)./(pi.*w4_t_out.^2./xlamdsout)).^2),'linestyle','--','color','b','linewidth',1.5);
    plot(ienergy,1e6.*w4_t_out./2.*sqrt(1+((z4_t_out-2)./(pi.*w4_t_out.^2./xlamdsout)).^2),'linestyle','-.','color','g','linewidth',1.5);
    plot(ienergy,1e6.*w1_out./2,'marker','*','color','k');
    hold off
    ylim(u2_plot_size_prop_lim);
    if numel(ienergy)>1
