...
realfield=0;
roughnessincluded=0;
abberationincluded=0;
disp('---------------------------------------------------');
disp(['realfield =',num2str(realfield)]);
disp(['roughness =',num2str(roughnessincluded)]);
disp(['abberation=',num2str(abberationincluded)]);
%showpictures=1;
 idlpl_tilt=(4:1:40).*1e-5;
 idlpl_tilt=10e-5;
...
Teta_g_i=1.04*0.01745;     %incidence angle new according to PRD        !!!!!!!
...
disp(['N=',num2str(sigma*1e-6*sqrt(2)/Teta_g_i/gr_sigma)]);
  ...
  if dTeta_d~=0
X=abs(X).*exp(1i*(angle(X)+dTeta_d*K*yy.*dx));
  end
...
X=lens(X.',leng,K,z1_prime,slitpos,f_g_t,f_g_s,R_g_tang,R_g_sag,Teta_g_i,Teta_g_d,D2*1e9,abberationincluded);
...
X=lens_tang(X,leng,K,z3_t,7.2-2.77-z3_t_offset,f_m_t,R_m_tang,R_m_tang,Teta_m_i,Teta_m_i,'y',abberationincluded); %f_m_t
...
%resolution=1/2/findHWHM(idlpl_tilt,inppowerout/max(inppowerout));
% 
