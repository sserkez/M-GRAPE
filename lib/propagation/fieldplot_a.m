...
function [H]=fieldplot_a(Nf,X,xlen,ylen,name,showpictures)
if showpictures
...
%         end
    ...
    if N~=1
        ...
        fig=figure(Nf);
                clf;
                set(fig,'name',[name,' (mesh=',num2str(round(Mx)),'x',num2str(round(My)),')'],'numbertitle','off');
                ...
                set(h2,'Ydir','reverse');
                axis square
                text(1,1,sprintf('FWHM = %.2e ', findFWHM(yscale,sum(abs(X).^2,2)')),...
                ...
                text(1,1,sprintf('FWHM = %.2e ', findFWHM(xscale,sum(abs(X).^2,1)')),...
                    'HorizontalAlignment','right','VerticalAlignment',...
                    'top','FontSize',10,'units','normalized');
