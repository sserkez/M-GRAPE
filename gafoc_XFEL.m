...
% % 
% % s1=(32.86-0.00863*energy)*1e-6/1.2; %sourse sigma
...
%trytrytrytrytrytry calculated waist size based on rms divergence
s1=(21.1-6.1*(energy-300)/700)*1e-6*sourcesize_x;
z1=z1-0.5;
s1=(20-5*(energy-300)/700)*1e-6*sourcesize_x;
s1=(14+(1239.8/energy-1.25)/(4.13-1.25)*(20-14))*1e-6*sourcesize_x;
z1=(0.6+0.6*(energy-300)/700)+U1_to_G;
%trytrytrytrytrytry calculated waist size based on FWHM divergence
% s1=12*1e-6*sourcesize_x;
% z1=-1.2;
...
w_slit=w2_t;% remove!!!! it causes resolving power at waist ? not slit, calculation!!
...
ang_disp_U2=xlamds*(z3_t_offset-f_m_t)/(Theta_g_d*gr_sigma*f_m_t);
